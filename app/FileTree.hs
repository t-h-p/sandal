...
buildOgFileTree arg = do
  code <- tryPath arg
  if code == 0
    then unfoldTreeM_BF createFsNode arg
    else error ("Exited with error code " ++ show code)



nodeTraverseTree :: Tree FsNode -> Int -> ([LNode String], Int)
nodeTraverseTree (Node fsNode subtrees) currentId =
    let currentNode = fsNodeToLNode currentId fsNode
        (childNodes, nextId) =
            foldl
            (\(accNodes, nextId) subtree ->
                let (subtreeNodes, newId) = nodeTraverseTree subtree nextId
...
buildLNodes tree = fst $ nodeTraverseTree tree 0
          ...
          (childEdges, nextId) =
            foldl
              (\(accEdges, nextId) subtree ->
                 let (subtreeEdges, newId) =
                       traverseTree subtree nextId (Just currentId)
